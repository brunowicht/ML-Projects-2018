...
def initialize_MF(train, n_features):
...
def update_user(train, item_features, lambda_user, nz_items_per_user, nz_user_itemindices):
    ...
    for user in range(n_users):
        nz_items = nz_items_per_user[user]
        nz_itemindices = nz_user_itemindices[user]
        nz_itemfeatures = item_features[nz_itemindices, :]
        
        A = (nz_itemfeatures.T).dot(nz_itemfeatures) + lambda_user * nz_items * np.eye(n_features)
...
def update_item(train, user_features, lambda_item, nz_users_per_item, nz_item_userindices):
    ...
    for item in range(n_items):
        nz_users = nz_users_per_item[item]
        nz_userindices = nz_item_userindices[item]
        nz_userfeatures = user_features[:, nz_userindices]
        A = (nz_userfeatures).dot(nz_userfeatures.T) + lambda_item * nz_users * np.eye(n_features)
...
def ALS(train, test, n_features, lambda_user, lambda_item, verbose=1):
    ...
    print('\nStarting ALS with n_features = %d, lambda_user = %f, lambda_item = %f' % (n_features, lambda_user, lambda_item))
    n_epochs = 10

    user_features_file_path = 'dump/user_features_%s_%s_%s_%s.npy'%(n_epochs, n_features, lambda_user, lambda_item)

    item_features_file_path = 'dump/item_features_%s_%s_%s_%s.npy'%(n_epochs, n_features, lambda_user, lambda_item)
    ...
    user_features, item_features = initialize_MF(train, n_features)
    ...
    nz_items_per_user = [len(i) for i in nz_user_itemindices]
    _, nz_item_userindices = map(list, zip(*nz_row_colindices))
    nz_users_per_item = [len(i) for i in nz_item_userindices]

    prev_train_rmse = 100

    for it in range(n_epochs):
        user_features = update_user(train,item_features, lambda_user, nz_items_per_user, nz_user_itemindices)

        item_features = update_item(train,user_features,lambda_item, nz_users_per_item,nz_item_userindices)

        train_rmse = helpers.calculate_rmse( np.dot(item_features, user_features)[train.nonzero()],train[train.nonzero()].toarray()[0])

        test_rmse = helpers.calculate_rmse( np.dot(item_features, user_features)[test.nonzero()], test[test.nonzero()].toarray()[0])
...
def get_ALS_predictions(ratings, train, test, n_features_array, lambda_user, lambda_item):
    ...
    for idx, n_features in enumerate(n_features_array):
        user_features, item_features = ALS(train, test, n_features, lambda_user, lambda_item)

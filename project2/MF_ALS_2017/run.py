...
X, X_train, y_train, X_test, y_test = get_ALS_predictions(ratings, train, test, n_features_array=range(1, 31),lambda_user=0.2,lambda_item=0.02)

# Linear blend of the previous models computed on the test set.
clf = get_linear_clf(X_test, y_test)
...
predicted_labels = clf.predict(X.T).round().astype(int).reshape(ratings.shape)
